...
async fn inner_async_main(hc_config: HcConfig) {
  ...
  let conductor =
      conductor_handle(hc_config.admin_ws_port, &hc_config.datastore_path, &hc_config.keystore_path).await;
  let _ = install_or_passthrough(&conductor, hc_config.app_id, hc_config.app_ws_port, hc_config.dnas).await;
...
async fn conductor_handle(
  ...
  let config = super::config::conductor_config(admin_ws_port, databases_path, keystore_path);
